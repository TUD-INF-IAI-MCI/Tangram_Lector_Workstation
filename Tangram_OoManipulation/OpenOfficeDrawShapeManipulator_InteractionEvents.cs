...
namespace tud.mci.tangram.TangramLector.SpecializedFunctionProxies
{
    ...
    public partial class OpenOfficeDrawShapeManipulator : AbstractSpecializedFunctionProxyBase
    {
        ...
        protected override void im_FunctionCall(object sender, FunctionCallInteractionEventArgs e)
        {
            ...
            {
                if (!InteractionManager.Instance.Mode.HasFlag(InteractionMode.Braille))
                {
                    switch (e.Function)
                    {
                        ...
                        case "unfocusElement":
                            ...
                            // clear the manipulation mode from the interaction manager
                            interactionManager.ChangeMode(interactionManager.Mode & ~InteractionMode.Manipulation);
                            playEdit();
            ...
            if (pointsObs != null)
            {
                ...
                if (!point.Equals(default(PolyPointDescriptor)))
                {
                    ...
                    String text = LL.GetTrans("tangram.oomanipulation.element_speaker.text.point",
                        ...
                        index,
                        count,
                         ((point.X / 1000f)).ToString("0.##cm"),
                         ((point.Y / 1000f)).ToString("0.##cm")
                        );
